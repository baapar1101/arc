...
افزونه **Hesabix V2 برای WooCommerce** با موفقیت ایجاد شد. این افزونه یک نسخه کاملاً بازنویسی شده از نسخه قدیمی است که با **API نسخه 2 مارک‌استریت** کار می‌کند و ویژگی‌های پیشرفته‌تری دارد.
...
```
1️⃣ Login با ایمیل و رمز عبور مارک‌استریت
...
```
WP Admin > مارک‌استریت V2 > همگام‌سازی

کلیک: "همگام‌سازی همه محصولات"
کلیک: "همگام‌سازی همه مشتریان"

✅ تمام داده‌ها به مارک‌استریت منتقل می‌شوند
```
...
کاربر محصول جدید ایجاد می‌کند
         ↓
افزونه خودکار محصول را به مارک‌استریت می‌فرستد
         ↓
...
مشتری سفارش ثبت می‌کند
         ↓
افزونه بررسی می‌کند: مشتری در مارک‌استریت هست؟
         ↓ (خیر)
مشتری ایجاد می‌شود
         ↓
...
محصولات همگام می‌شوند
         ↓
فاکتور فروش در مارک‌استریت ایجاد می‌شود
         ↓
شماره فاکتور به سفارش اضافه می‌شود
         ↓
✅ موفق - فاکتور در مارک‌استریت ثبت شد
```
...
بررسی: sync_on_product_update فعال است؟
         ↓ (بله)
محصول در مارک‌استریت به‌روزرسانی می‌شود
         ↓
...
- ✅ چند فروشگاه با یک حساب مارک‌استریت
...
**توسعه‌دهنده:** MarkStreet Team با کمک AI  
**تاریخ:** 5 دسامبر 2024  
**زمان توسعه:** 4 ساعت  
