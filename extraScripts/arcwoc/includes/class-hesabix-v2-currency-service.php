...
=======
	 * ارزیابی هم‌خوانی ارز ووکامرس با ارز فاکتور مارک‌استریت و ضریب تبدیل مبلغ.
>>>>>>> github/Huma
	 *
	 * @param Hesabix_V2_Api|null $api
	...
	public static function evaluate_currency_sync(Hesabix_V2_Api $api = null, $wc_currency_override = null)
	{
		if (!get_option('hesabix_v2_enabled')) {
			return array(
				...
				'factor' => 1.0,
				'currency_id' => 0,
				'message' => __('همگام‌سازی مارک‌استریت غیرفعال است.', 'hesabix-v2'),
		...
		if (!$row) {
			return array(
				...
				'currency_id' => 0,
				'message' => __(
					'لیست ارزهای کسب‌وکار از مارک‌استریت دریافت نشد یا ارز انتخاب‌شده در افزونه دیگر برای این کسب‌وکار فعال نیست. تنظیمات «ارز فاکتور» را بررسی کنید.',
					'hesabix-v2'
				),
				'currency_blocked' => true,
			);
		...
		return array(
			...
			'message' => sprintf(
				...
				__(
					'ارز فروشگاه ووکامرس (%1$s) با ارز فاکتور انتخاب‌شده در مارک‌استریت (%2$s — %3$s) هم‌خوان نیست. تا یکسان کردن ارزها یا استفاده از «تومان» در ووکامرس و «ریال (IRR)» در مارک‌استریت، همگام‌سازی متوقف می‌ماند.',
					'hesabix-v2'
				),
				$wc,
				$hx_code,
