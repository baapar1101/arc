<?php
/**
 * وضعیت استثناء سفارش از همگام‌سازی خودکار با مارک‌استریت (مثلاً پس از ویرایش دستی فاکتور).
 *
 * @package    Hesabix_V2
...
class Hesabix_V2_Order_Sync_Meta
{
	...
	/**
	 * متا برای شناسهٔ نظر واحد «وضعیت همگام‌سازی مارک‌استریت»: ارز، سال مالی، خطای فاکتور؛ با هر رویداد همان نظر به‌روز می‌شود.
	 */
	...
	public static function set_or_update_invoice_sync_error_note($order, $exception_message)
	{
		...
		$content = sprintf(
			__('خطا در ایجاد فاکتور مارک‌استریت: %s', 'hesabix-v2'),
			$body
		);
