...
### Fixed
- فهرست مشتریان پل REST همیشه خالی بود: `WC_Customer_Query` در ووکامرس جدید وجود ندارد؛ کوئری با `WP_User_Query` و نقش‌های customer/subscriber جایگزین شد
- جلوگیری از کش شدن پاسخ‌های پل توسط LiteSpeed با هدر `X-LiteSpeed-Cache-Control: no-cache`
- شمارش مشتریان در گزارش خلاصه هم‌راستا با نقش‌های همگام‌سازی افزونه

## [4.9.2] - 2026-08-04

### Fixed
- نمایش سود با واحد ارز فاکتور حسابیکس (مثلاً ریال ایران) به‌جای قالب‌بندی اشتباه با ارز ووکامرس (تومان)

## [4.9.1] - 2026-08-04

### Fixed
- خواندن سود از `data.item` در پاسخ GET فاکتور (قبلاً به‌اشتباه صفر ذخیره می‌شد)

### Added
- هشدار در متاباکس وقتی بهای تمام‌شده صفر است و سود نمایشی برابر فروش است

## [4.9.0] - 2026-08-04

### Added
- نمایش سود فاکتور حسابیکس در فهرست سفارش‌های افزونه و متاباکس ویرایش سفارش ووکامرس
- دریافت خودکار سود پس از همگام‌سازی موفق فاکتور؛ دکمهٔ تازه‌سازی تکی و دسته‌ای
- بنر دعوت به فعال‌سازی لایسنس افزونهٔ رسمی ووکامرس در بازار حسابیکس (پنهان هنگام لایسنس فعال)
- یادآوری فعال‌سازی پل وردپرس وقتی لایسنس هست ولی پل آماده نیست
- کش وضعیت لایسنس بازار افزونه با امکان بررسی مجدد و پنهان‌سازی موقت بنر

## [4.8.2] - 2026-08-04

### Fixed
- بارگذاری خالی لیست حساب بانکی و صندوق در تنظیمات: درخواست با `take=500` از سقف `QueryInfo` API (حداکثر ۱۰۰) رد می‌شد و افزونه بدون پیام خطا لیست خالی نشان می‌داد
- نمایش پیام خطای API برای بانک/صندوق/انبار به‌جای موفقیت خاموش با لیست خالی
- پاک‌سازی خودکار شناسه بانک/صندوق پیش‌فرض که به کسب‌وکار فعلی تعلق ندارد (جلوگیری از ثبت دریافت روی حساب کسب‌وکار دیگر)
