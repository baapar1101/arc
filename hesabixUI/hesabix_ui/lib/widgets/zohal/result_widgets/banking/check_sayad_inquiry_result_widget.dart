...
class CheckSayadInquiryResultWidget extends ZohalResultWidget {
  ...
  List<Widget> buildResultContent(BuildContext context) {
    ...
    final issueDate = data['issue_date']?.toString() ?? '';
      ...
      Container(
        ...
        child: Column(
          ...
          children: [
            ...
            if (name.isNotEmpty)
              _buildInfoRow(context, theme, 'نام صادرکننده', name),
            if (sayadId.isNotEmpty)
              _buildInfoRow(
                context,
                theme,
                'شناسه صیادی',
            ...
            if (checkType.isNotEmpty)
              _buildInfoRow(context, theme, 'نوع چک', checkType),
            if (serialNo.isNotEmpty)
              _buildInfoRow(context, theme, 'شماره سریال', serialNo),
            if (seriesNo.isNotEmpty)
              _buildInfoRow(context, theme, 'سری چک', seriesNo),
            if (issueDate.isNotEmpty)
              _buildInfoRow(context, theme, 'تاریخ صدور', issueDate),
            if (expirationDate != null && expirationDate.isNotEmpty)
              _buildInfoRow(context, theme, 'تاریخ سررسید', expirationDate),
            if (branchCode.isNotEmpty)
              _buildInfoRow(context, theme, 'کد شعبه', branchCode),
