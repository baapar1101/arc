...
class WarehouseDocumentFormDialog extends StatefulWidget {
  ...
  @override
  State<WarehouseDocumentFormDialog> createState() =>
      _WarehouseDocumentFormDialogState();
}

class _WarehouseDocumentFormDialogState
    extends State<WarehouseDocumentFormDialog> {
  ...
  final Map<int, List<Map<String, dynamic>>> _productAttributesCache =
  ...
  Map<String, dynamic> _resolveLineWarehouse(
    ...
    final resolvedWarehouse =
        lineWarehouse ?? _defaultWarehouseForMovement(movement);
    return {
  ...
  Widget _buildSourceBanner() {
    ...
    final invoiceLabel =
        widget.sourceInvoiceCode ?? '#${widget.sourceInvoiceId}';
      ...
      child: ListTile(
        ...
        subtitle: Text(
          typeLabel.isNotEmpty
              ? 'نوع فاکتور: $typeLabel'
              : 'شناسه: ${widget.sourceInvoiceId}',
        ),
      ),
    );
  }
  ...
  void initState() {
    ...
    if (_isEditMode) {
      ...
      for (final raw in widget.initialLines!) {
        ...
        normalized['extra_info'] = Map<String, dynamic>.from(
          normalized['extra_info'] ?? const {},
        );
      ...
      try {
        ...
        if (invoiceItem != null) {
          ...
          if (extraInfo != null) {
            ...
            if (whId != null) {
              invoiceDocumentWarehouseId = (whId is num)
                  ? whId.toInt()
                  : int.tryParse(whId.toString());
            }
          }
        }
      } catch (e) {
    ...
    try {
      ...
      if (status != 'draft' && status != 'posted') {
        if (mounted) {
          SnackBarHelper.show(
            context,
            message: 'فقط حواله‌های پیش‌نویس و قطعی شده قابل ویرایش هستند',
          );
      ...
      if (status == 'posted') {
        if (mounted) {
          SnackBarHelper.show(
            context,
            message:
                'برای حواله‌های قطعی شده، فقط اطلاعات کالاهای یونیک قابل ویرایش است',
          );
        }
      }
      ...
      _description =
          item['description'] as String? ?? extraInfo['description'] as String?;
      _deliveryMethod =
          item['delivery_method'] as String? ??
          extraInfo['delivery_method'] as String?;
      _carrierName =
          item['carrier_name'] as String? ??
          extraInfo['carrier_name'] as String?;
      _recipientName =
          item['recipient_name'] as String? ??
          extraInfo['recipient_name'] as String?;
      _recipientPhone =
          item['recipient_phone'] as String? ??
          extraInfo['recipient_phone'] as String?;
      _trackingNumber =
          item['tracking_number'] as String? ??
          extraInfo['tracking_number'] as String?;
      ...
      for (final line in lines) {
        ...
        lineMap['extra_info'] = Map<String, dynamic>.from(
          lineMap['extra_info'] ?? const {},
        );
        ...
        if (lineMap['instance_data'] == null &&
            lineMap['instance_ids'] != null) {
  ...
  /// فیلدهای ارسال؛ در تب موبایل [forceStacked] true تا همیشه عمودی باشد.
  Widget _buildShippingFormFields({bool forceStacked = false}) {
    Widget deliveryBlock(BuildContext ctx, double maxW) {
      final narrow = forceStacked || maxW < 600;
      if (narrow) {
        return Column(
          ...
          children: [
            DropdownButtonFormField<String>(
              ...
              items: const [
                DropdownMenuItem(
                  value: 'warehouse_door',
                  child: Text('تحویل درب انبار'),
                ),
                DropdownMenuItem(
                  value: 'post_regular',
                  child: Text('پست عادی'),
            ...
            if (_showCarrierName) ...[
              ...
              TextFormField(
                ...
                onChanged: (value) =>
                    setState(() => _carrierName = value.isEmpty ? null : value),
              ),
            ],
          ],
        );
    ...
    Widget recipientBlock(double maxW) {
      final narrow = forceStacked || maxW < 600;
      if (narrow) {
        return Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            TextFormField(
              ...
              onChanged: (value) =>
                  setState(() => _recipientName = value.isEmpty ? null : value),
            ),
            const SizedBox(height: 12),
            TextFormField(
              ...
              onChanged: (value) => setState(
                () => _recipientPhone = value.isEmpty ? null : value,
              ),
            ),
          ],
        );
    ...
    return Column(
      ...
      children: [
        TextFormField(
          ...
          maxLines: 2,
          onChanged: (value) =>
              setState(() => _description = value.isEmpty ? null : value),
        ),
        const SizedBox(height: 12),
        if (forceStacked) ...[
          deliveryBlock(context, 0),
          const SizedBox(height: 12),
          recipientBlock(0),
        ] else
          LayoutBuilder(
            builder: (context, constraints) {
              return Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  deliveryBlock(context, constraints.maxWidth),
                  const SizedBox(height: 12),
                  recipientBlock(constraints.maxWidth),
                ],
              );
            },
          ),
        ...
        TextFormField(
          ...
          onChanged: (value) =>
              setState(() => _trackingNumber = value.isEmpty ? null : value),
        ),
      ],
    );
  }
  ...
  Widget _buildShippingInfoSection() {
    return ExpansionTile(
      ...
      children: [_buildShippingFormFields(forceStacked: false)],
    );
  }
                ...
                children: [
                  DropdownButtonFormField<String>(
                    ...
                    items: const [
                      DropdownMenuItem(
                        value: 'receipt',
                        child: Text('حواله ورود'),
                      ),
                      DropdownMenuItem(
                        value: 'issue',
                        child: Text('حواله خروج'),
                      ),
                      DropdownMenuItem(
                        value: 'transfer',
                        child: Text('انتقال بین انبارها'),
                      ),
                      DropdownMenuItem(
                        value: 'adjustment',
                        child: Text('تعدیل موجودی'),
                      ),
                      DropdownMenuItem(
                        value: 'production_in',
                        child: Text('ورود تولید'),
                      ),
                      DropdownMenuItem(
                        value: 'production_out',
                        child: Text('خروج تولید'),
                      ),
                    ],
                    ...
                    validator: (value) =>
                        value == null ? 'لطفاً نوع حواله را انتخاب کنید' : null,
                  ),
                  const SizedBox(height: 12),
                    ...
                    Expanded(
                      ...
                      child: DropdownButtonFormField<String>(
                        ...
                        validator: (value) => value == null
                            ? 'لطفاً نوع حواله را انتخاب کنید'
                            : null,
                      ),
                    ),
                    const SizedBox(width: 12),
  ...
  Widget _buildLinesToolbar({required bool mobileLayout}) {
    if (mobileLayout) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Text('خطوط حواله', style: Theme.of(context).textTheme.titleMedium),
  ...
  Widget _buildLineProductAndActions(
    int index, {
    required bool compactActions,
  }) {
    final line = _lines[index];
    final productWidget = ProductComboboxWidget(
      key: ValueKey('product_line_$index'),
      businessId: widget.businessId,
      selectedProduct: () {
        ...
        final selectedProduct = productId != null
            ? {'id': productId, 'name': productName, 'code': productCode}
            : null;
    ...
    void onMenu(String? value) {
      if (value == 'unique_in') {
        _registerUniqueProductInstances(index);
      } else if (value == 'unique_out') {
        _selectUniqueProductInstances(index);
      } else if (value == 'auto_line') {
        _autoCompleteLine(index);
      } else if (value == 'delete') {
        _removeLine(index);
      }
    }

    final iconRow = <Widget>[
      ...
      entries.add(
        const PopupMenuItem(
          ...
          child: ListTile(
            ...
            title: Text('تکمیل خودکار این خط'),
          ),
        ),
      );
    }
  ...
  Future<void> _registerUniqueProductInstances(int lineIndex) async {
    ...
    final existingInstances =
        (line['instance_data'] as List<dynamic>?)
    ...
    if (result != null) {
      ...
      if (result.length > quantity) {
        SnackBarHelper.showError(
          context,
          message:
              'تعداد کالاهای یونیک ثبت شده (${result.length}) نمی‌تواند از تعداد (${quantity}) بیشتر باشد',
        );
        return;
      }
      ...
      if (currentQuantity != null && currentQuantity > 0) {
        if (selectedIds.length > currentQuantity) {
          SnackBarHelper.showError(
            context,
            message:
                'تعداد کالاهای یونیک انتخاب شده (${selectedIds.length}) نمی‌تواند از تعداد (${currentQuantity}) بیشتر باشد',
          );
          return;
        }
        // اگر quantity از قبل تنظیم شده، آن را حفظ می‌کنیم
        _updateLine(lineIndex, {'instance_ids': selectedIds});
      } else {
  ...
  Future<void> _save() async {
    ...
    if (_docType == 'transfer') {
      if (_warehouseIdFrom == null) {
        SnackBarHelper.show(
          context,
          message: 'لطفاً انبار مبدا را انتخاب کنید',
        );
        return;
      }
      if (_warehouseIdTo == null) {
        SnackBarHelper.show(
          context,
          message: 'لطفاً انبار مقصد را انتخاب کنید',
        );
        return;
      }
    } else if (_docType == 'issue' || _docType == 'production_out') {
    ...
    for (var i = 0; i < _lines.length; i++) {
      ...
      if (line['product_id'] == null) {
        SnackBarHelper.show(
          context,
          message: 'خط ${i + 1}: لطفاً محصول را انتخاب کنید',
        );
        return;
      }
          ...
          if (instanceIds != null && instanceIds.isNotEmpty) {
            if (instanceIds.length > quantity) {
              SnackBarHelper.showError(
                context,
                message:
                    'خط ${i + 1}: تعداد کالاهای یونیک انتخاب شده (${instanceIds.length}) نمی‌تواند از تعداد (${quantity}) بیشتر باشد',
              );
              return;
            }
          }
          ...
          if (instanceData != null && instanceData.isNotEmpty) {
            if (instanceData.length > quantity) {
              SnackBarHelper.showError(
                context,
                message:
                    'خط ${i + 1}: تعداد کالاهای یونیک ثبت شده (${instanceData.length}) نمی‌تواند از تعداد (${quantity}) بیشتر باشد',
              );
              return;
            }
          }
  ...
  Future<void> _saveManual() async {
    final payload = {
      ...
      if (_carrierName != null && _carrierName!.isNotEmpty)
  ...
  /// یک خط حواله (کارت) — برای دسکتاپ و تب موبایل
  Widget _warehouseLineCard(int index, {required bool compactActions}) {
    final line = _lines[index];
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            _buildLineProductAndActions(index, compactActions: compactActions),
                    ...
                    ...(line['instance_data'] as List<dynamic>)
                        ...
                        .map((entry) {
                          ...
                          if (attrs != null && attrs.isNotEmpty) {
                            ...
                            final productId = line['product_id'] as int?;
                            final productAttrs = productId != null
                                ? _productAttributesCache[productId]
                                : null;

                            if (productAttrs != null &&
                                productAttrs.isNotEmpty) {
                              // ساخت attributesMap
            ...
            Builder(
              builder: (context) {
                final instanceIds = line['instance_ids'] as List?;
                final instanceData = line['instance_data'] as List?;
                final hasInstanceIds =
                    instanceIds != null && instanceIds.isNotEmpty;
                final hasInstanceData =
                    instanceData != null && instanceData.isNotEmpty;
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              hasInstanceData
                                  ? 'اطلاعات ${instanceData?.length ?? 0} کالای یونیک ثبت شده'
                                  : '${instanceIds?.length ?? 0} کالای یونیک انتخاب شده',
                              style: Theme.of(context).textTheme.bodyMedium
                                  ?.copyWith(
                                    fontWeight: FontWeight.bold,
                                    color: Theme.of(
            ...
            if (_docType != 'adjustment' && _docType != 'transfer') ...[
              ...
              Builder(
                builder: (context) {
                  final movement =
                      (line['movement'] as String?) ??
                      _movementForDocType(_docType);
                  final resolved = _resolveLineWarehouse(line, movement);
                  final warehouseId = resolved['warehouse_id'] as int?;
                  final isFromDocumentLevel =
                      resolved['is_from_document_level'] as bool? ?? false;
            ...
            if (_docType == 'transfer') ...[
              ...
              Builder(
                builder: (context) {
                  ...
                  final whFrom = resolved['warehouse_id_from'] as int?;
                  final whTo = resolved['warehouse_id_to'] as int?;
                  final isFromDocLevelFrom =
                      resolved['is_from_document_level_from'] as bool? ?? false;
                  final isFromDocLevelTo =
                      resolved['is_from_document_level_to'] as bool? ?? false;
                        ...
                        appBar: AppBar(
                          ...
                          bottom: TabBar(
                            ...
                            tabs: const [
                              ...
                              Tab(text: 'خطوط', icon: Icon(Icons.list_alt)),
                              Tab(
                                text: 'ارسال',
                          ...
                          children: [
                            ...
                            SingleChildScrollView(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.stretch,
                                children: [
                                  _buildLinesToolbar(mobileLayout: true),
                                  const SizedBox(height: 8),
                                  if (_lines.isEmpty)
                                    Card(
                                      child: Padding(
                            ...
                            SingleChildScrollView(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 12),
                                  _buildShippingFormFields(forceStacked: true),
                                ],
                              ),
                            ),
                          ],
                          ...
                          child: SingleChildScrollView(
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 12),
                                _buildLinesToolbar(mobileLayout: false),
                                if (_lines.isEmpty)
                                  const Padding(
                                    padding: EdgeInsets.all(16),
                                    child: Center(
                                      child: Text('هیچ خطی وجود ندارد'),
                                    ),
                                  )
                                else
                                  ...List.generate(
                                    ...
                                    (index) => _warehouseLineCard(
                                      index,
                                      compactActions: ResponsiveHelper.isMobile(
                                        context,
                                      ),
                                    ),
                                  ),
...
class _ProductInstancesManagerDialog extends StatefulWidget {
  ...
  @override
  State<_ProductInstancesManagerDialog> createState() =>
      _ProductInstancesManagerDialogState();
}

class _ProductInstancesManagerDialogState
    extends State<_ProductInstancesManagerDialog> {
