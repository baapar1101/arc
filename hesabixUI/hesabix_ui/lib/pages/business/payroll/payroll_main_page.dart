    ...
    return Scaffold(
      ...
      floatingActionButton: _canOperate && _tabController.index == 0
          ...
          : (_canOperate && _tabController.index == 1
              ? FloatingActionButton.extended(
                  onPressed: () => _PeriodsTab.showCreateDialog(context, widget.businessId, _loadAll),
                  icon: const Icon(Icons.add),
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, i) {
        ...
        return ListTile(
          ...
          subtitle: Text('${run['run_date'] ?? ''} · ${run['status'] ?? ''}'),
  ...
  static Future<void> showCreateDialog(
    ...
    try {
      await PayrollService().createPeriod(
        ...
        payload: {
          'year': int.tryParse(yearCtrl.text.trim()) ?? now.year,
          'month': int.tryParse(monthCtrl.text.trim()) ?? now.month,
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, i) {
        ...
        return ListTile(
          ...
          subtitle: Text(
            '${period['year']}/${period['month']} · ${_statusLabel(period['status'] as String?)}',
          ),
...
class _EmployeesTab extends StatelessWidget {
  ...
  Future<void> _addEmployee(BuildContext context) async {
    final payload = await PayrollEmployeeFormDialog.show(context, businessId: businessId);
    if (payload == null) return;
    try {
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    return ListTile(
                      ...
                      subtitle: Text('${emp['job_title'] ?? ''} · ${emp['employee_code'] ?? ''}'),
