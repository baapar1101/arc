...
- همه تاریخ‌های UI (تاریخ سند، استخدام، پایان همکاری، بازه دوره، اسناد حسابداری) از `DateInputField` و `PayrollCalendarUtils` استفاده می‌کنند.
...
- افزودن شخص جدید از combobox: نوع شخص مطابق فیلتر combobox از پیش انتخاب می‌شود (`PersonFormDialog.initialPersonTypes`).
