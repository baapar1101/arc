...
def normalize_moadian_unit_code(raw: str | int | None) -> str:
    """
    واحد اندازه‌گیری مودیان باید کد عددی باشد (الگوی ^\\d{0,8}$).
    در داده‌های قدیمی/seed اغلب نام فارسی (مثل «عدد») ذخیره شده است.
    """
    if raw is None:
        return "1627"  # عدد — مطابق نمونه SDK PHP
    s = str(raw).strip()
    if not s:
        return "1627"
    if s.isdigit():
    ...
    s_norm = s.replace("ي", "ی").replace("ك", "ک").replace("_", " ").strip()

    # نگاشت رایج نام‌ها → کدهای سامانه مودیان
    name_to_code = {
        "عدد": "1627",
        "کیلوگرم": "1611",
        "کیلو گرم": "1611",
    ...
    return name_to_code.get(s_norm, "1627")


def calculate_vat_rate(tax_percentage: float) -> int:
    """
    تبدیل درصد مالیات به فرمت سامانه مودیان / SDK PHP.

    طبق مستند رسمی، vra عدد اعشاری با حداکثر ۲ رقم اعشار است (مثلاً ۹ یا ۱۰).
    کتابخانه Snapp-Market و نسخهٔ قدیمی حسابیکس هم ۹٪ را به صورت ۹ می‌فرستند
    (نه ۹۰۰).

    ورودی می‌تواند «۹» / «۱۰» (درصد) یا «۰.۰۹» (کسر) باشد.
    """
    try:
    ...
    if tf <= 0:
        return 0
    # کسر اعشاری نمایندهٔ درصد (مثلاً ۰.۰۹ → ۹)؛ مقدار ۱ به‌عنوان ۱٪ حفظ می‌شود
    if 0 < tf < 1.0:
        tf = tf * 100.0
    # مقادیر اشتباه قدیمی (×۱۰۰ مثل ۹۰۰ برای ۹٪) را به درصد واقعی برمی‌گردانیم
    if tf >= 100.0:
        tf = tf / 100.0
    return int(round(tf))
...
def extract_moadian_error_message(error_data: dict) -> str:
    ...
    error_map = {
        ...
        '4103': (
            'کد ملی گواهی امضا با شناسه کلاینت توکن مطابقت ندارد. '
            'اگر در نسخه قبلی (PHP) فقط با کلید خصوصی ارسال می‌کردید، فیلد «گواهی» را خالی کنید '
            'یا گواهی صادرشده از CSR همین کلید را بارگذاری کنید.'
        ),
        'TAX_CERTIFICATE_REQUIRED': 'گواهی امضای الکترونیک در تنظیمات ثبت نشده است',
        'TAX_CERTIFICATE_INVALID': 'گواهی امضا نامعتبر است یا توسط مرکز میانی صادر نشده',
