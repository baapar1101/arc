"""
ایجاد/ویرایش گروهی اسناد هزینه و درآمد برای مهاجرت و ادغام خارجی.
...
from app.services.expense_income_service import create_expense_income, update_expense_income

MAX_BULK_EXPENSE_INCOME_ITEMS = 200
...
def bulk_upsert_expense_income_integration(
    db: Session,
    business_id: int,
    ...
    if len(items) > MAX_BULK_EXPENSE_INCOME_ITEMS:
        raise ApiError(
            "BULK_TOO_LARGE",
            f"حداکثر {MAX_BULK_EXPENSE_INCOME_ITEMS} آیتم در هر درخواست مجاز است",
            http_status=400,
        )
    ...
    if migration_mode:
        if not has_business_permission_for_business(
            auth_context, db, business_id, "expenses_income", "add"
        ):
            raise ApiError("FORBIDDEN", "مجوز مهاجرت هزینه/درآمد وجود ندارد", http_status=403)
        ...
        for idx, raw in enumerate(items):
            ...
            if document_id_val is None:
                    ...
                    results.append(
                        {
                            **base,
                            ...
                            "message": "مجوز افزودن هزینه/درآمد وجود ندارد",
                        }
                    )
                    continue
                ...
                if doc_type not in _ALLOWED_TYPES:
                    ...
                    continue
                try:
                    created = create_expense_income(db, business_id, int(uid), payload)
                ...
                if not has_business_permission_for_business(
                    auth_context, db, business_id, "expenses_income", "edit"
                ):
                    results.append(
                        {
                            **base,
                            ...
                            "error_code": "FORBIDDEN",
                            "message": "مجوز ویرایش هزینه/درآمد وجود ندارد",
                        }
                    )
                    continue
                    ...
                    results.append(
                        {
                            **base,
                            ...
                            "message": "سند هزینه/درآمد یافت نشد",
                        }
                    )
                    continue
                try:
                    updated = update_expense_income(db, document_id_val, int(uid), payload)
