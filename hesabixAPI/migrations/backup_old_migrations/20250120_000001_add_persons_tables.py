...
def downgrade() -> None:
    ...
    if 'person_bank_accounts' in tables:
        existing_indexes = {idx['name'] for idx in inspector.get_indexes('person_bank_accounts')}
        if 'ix_person_bank_accounts_person_id' in existing_indexes:
            try:
    ...
    if 'persons' in tables:
        existing_indexes = {idx['name'] for idx in inspector.get_indexes('persons')}
        if 'ix_persons_national_id' in existing_indexes:
            try:
                op.drop_index(op.f('ix_persons_national_id'), table_name='persons')
            except Exception:
                pass
        if 'ix_persons_alias_name' in existing_indexes:
            try:
                op.drop_index(op.f('ix_persons_alias_name'), table_name='persons')
            except Exception:
                pass
        if 'ix_persons_business_id' in existing_indexes:
            try:
