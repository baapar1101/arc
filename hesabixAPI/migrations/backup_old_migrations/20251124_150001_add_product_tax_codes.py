...
def upgrade() -> None:
    ...
    table_exists = "product_tax_codes" in inspector.get_table_names()
    ...
    index_names = set()
    if table_exists:
        index_names = {index["name"] for index in inspector.get_indexes("product_tax_codes")}

    if not table_exists or "ix_product_tax_codes_code" not in index_names:
