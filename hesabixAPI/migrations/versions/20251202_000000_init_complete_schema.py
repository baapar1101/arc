...
def _column_exists(conn, table_name: str, column_name: str) -> bool:
    ...
    conn = op.get_bind()
        ...
        pass  # اگر از قبل وجود داشته باشد، نادیده بگیر
    ...
    if 'otp_login_sessions' not in tables:
        pass
    if 'otp_login_sessions' not in tables:
                sa.Column('last_otp_sent_at', sa.DateTime(), nullable=True),  # Added for rate limiting
            ...
            op.create_index(op.f('ix_otp_login_sessions_email'), 'otp_login_sessions', ['email'], unique=False)  # Added email index
