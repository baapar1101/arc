...
class AIPromptRepository(BaseRepository[AIPrompt]):
    ...
    def get_user_prompts(
        ...
        """دریافت تمام prompt های شخصی کاربر"""
        return self.db.query(self.model_class).filter(
            and_(
                self.model_class.user_id == user_id,
                self.model_class.is_active == True  # noqa: E712
    ...
    def get_all_default_prompts(self) -> List[AIPrompt]:
        """دریافت تمام prompt های پیش‌فرض"""
        return self.db.query(self.model_class).filter(
            and_(
                self.model_class.is_default == True,  # noqa: E712
                self.model_class.user_id == None,  # noqa: E711
        ...
        ).order_by(self.model_class.role, self.model_class.prompt_type).all()
