...
from app.services.product_opening_balance_service import get_product_opening_balance_eligibility
...
@router.get(
    "/businesses/{business_id}/opening-balance/product-line-eligibility",
    summary="وضعیت ثبت تعداد اولیه کالا",
    description="بررسی امکان نمایش/ثبت تعداد اولیه در فرم افزودن یا ویرایش کالا",
)
async def get_product_opening_balance_eligibility_endpoint(
    request: Request,
    business_id: int,
    fiscal_year_id: Optional[int] = Query(None, description="شناسه سال مالی (در غیر این صورت سال جاری)"),
    product_id: Optional[int] = Query(None, description="شناسه کالا (برای فرم ویرایش)"),
    warehouse_id: Optional[int] = Query(None, description="شناسه انبار (برای یافتن خط موجود)"),
    db: Session = Depends(get_db),
    ctx: AuthContext = Depends(get_current_user),
    _: None = Depends(require_business_permission_dep("opening_balance", "view")),
):
    can_edit = has_business_permission_for_business(ctx, db, business_id, "opening_balance", "edit")
    result = get_product_opening_balance_eligibility(
        db,
        business_id,
        fiscal_year_id,
        can_edit_opening_balance=can_edit,
    ...
    return success_response(
        data=format_datetime_fields(result, request),
        request=request,
        message="PRODUCT_OPENING_BALANCE_ELIGIBILITY",
    )
